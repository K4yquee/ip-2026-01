...
// UpdatePatient atualiza os dados de um paciente no banco de dados com base em um mapa de alterações
func UpdatePatient(currentEmail string, updates map[string]string) error {
    // Se não houver atualizações, retorna direto
    if len(updates) == 0 {
        return nil
    }
    ...
    // Adiciona o email atual como condição para identificar o paciente a ser atualizado
    values = append(values, currentEmail)
    // Constrói a query SQL completa com os trechos de atualização e a condição
    query := fmt.Sprintf("UPDATE patients SET %s WHERE email = $%d", strings.Join(setClauses, ", "), i)
